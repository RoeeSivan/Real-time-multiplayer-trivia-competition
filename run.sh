...
EOF
  ...
  export FRONTEND_URL="$FRONTEND_URL_TUN"
fi
...
if [[ $MODE == "tunnel" ]]; then
  printf "\n%s[run]%s %sTunnels live:%s\n" "$C_BLUE" "$C_RESET" "$C_GREEN" "$C_RESET"
  printf "  host (open this on your laptop): %s%s/host%s\n" "$C_PURPLE" "$FRONTEND_URL_TUN" "$C_RESET"
  printf "  solo:                            %s%s/cpu%s\n" "$C_PURPLE" "$FRONTEND_URL_TUN" "$C_RESET"
  printf "  backend API:                     %s%s%s\n" "$C_PURPLE" "$BACKEND_URL" "$C_RESET"
  printf "  ngrok inspector:                 %shttp://127.0.0.1:4040%s\n\n" "$C_PURPLE" "$C_RESET"
fi
